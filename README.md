...
│                                           ▼                   │                           │
│                                    ConnectionManage           ▼                           │
│                                  (Client Sessions TCP) Single-Thread Writer               │
│                                           │                   │                           │
│                                           ▼                   ▼                           │
...
* **Phát sự kiện thời gian thực (Observer Pattern):** Song song với hàng đợi ghi đĩa, một sự kiện `NEW_BID` được gửi qua `AuctionEventBus`. Lớp `LiveRoomManage` (Observer) bắt sự kiện này và ngay lập tức gửi thông điệp cập nhật cho tất cả client trong phòng thông qua socket TCP (`ConnectionManage`), đảm bảo độ trễ cập nhật trên màn hình người dùng là dưới 10ms.
...
| Kiểm thử | JUnit 5 · Mockito 5 · 693 test cases |
